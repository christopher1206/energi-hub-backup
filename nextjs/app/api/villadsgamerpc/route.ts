...
const SHELLY_TOPIC = 'VilladsGamerPC';
  ...
  try {
    ...
    const liveFlux = `
      from(bucket: "${INFLUX_BUCKET}")
        |> range(start: -6h)
        |> filter(fn: (r) => r._measurement == "villadsgamerpc")
        |> last()
    ...
    const energiFlux = `
      from(bucket: "${INFLUX_BUCKET}")
        ...
        |> filter(fn: (r) => r._measurement == "villadsgamerpc" and r._field == "apower")
...
export async function POST(request: Request) {
  try {
    ...
    const result = await new Promise((resolve, reject) => {
      ...
      const src = 'villadsgamerpc-dashboard';
