...
const SHELLY_TOPIC = 'Vinkolerskab';
  ...
  try {
    ...
    const liveFlux = `
      from(bucket: "${INFLUX_BUCKET}")
        |> range(start: -6h)
        |> filter(fn: (r) => r._measurement == "vinkoelerskab")
        |> last()
    ...
    const energiFlux = `
      from(bucket: "${INFLUX_BUCKET}")
        ...
        |> filter(fn: (r) => r._measurement == "vinkoelerskab" and r._field == "apower")
...
export async function POST(request: Request) {
  try {
    ...
    const result = await new Promise((resolve, reject) => {
      ...
      const src = 'vinkoelerskab-dashboard';
