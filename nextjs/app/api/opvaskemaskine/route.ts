...
const SHELLY_TOPIC = 'shellyplugsg3-d885ac1c3514';
  ...
  try {
    ...
    const liveFlux = `
      from(bucket: "${INFLUX_BUCKET}")
        |> range(start: -6h)
        |> filter(fn: (r) => r._measurement == "opvaskemaskine")
        |> last()
    ...
    const energiFlux = `
      from(bucket: "${INFLUX_BUCKET}")
        ...
        |> filter(fn: (r) => r._measurement == "opvaskemaskine" and r._field == "apower")
...
export async function POST(request: Request) {
  try {
    ...
    const result = await new Promise((resolve, reject) => {
      ...
      const src = 'opvaskemaskine-dashboard';
